...
std::vector<Question *> randomize_questions(std::vector<Question *> questions) {
  std::vector<Question *> randomizedQuestions;
  int i = 0;
  #pragma omp parallel for
  for (i; i < (int) size(questions); i++) {
    randomizedQuestions.push_back(questions[i]);
  }
...
Database::Database() {
  ...
  std::cout << _questions[0]->getDescription() << "/n";
  _questions = randomize_questions(_questions);
  std::cout << _questions[0]->getDescription();
